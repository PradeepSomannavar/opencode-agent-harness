    ...
    changedFilesStorePromise = import("../plugins/lib/changed-files-store.js").catch(() => {
      ...
      throw new Error(
        "changed-files tool: could not load the changed-files store. " +
          "This usually means the plugin's `lib` directory is missing or incomplete " +
          "(an interrupted or partial install can leave tools/ populated without plugins/). " +
          "Reinstall the opencode-agent-harness package to restore the missing files."
      )
    })
  }
