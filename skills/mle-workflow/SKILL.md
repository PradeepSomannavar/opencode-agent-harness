...
- `mle-reviewer` when a production ML system needs review (data contracts, reproducibility, evals, serving, monitoring, rollback)
- `eval-harness` and `ai-regression-testing` for promotion gates and agent-assisted regression checks
- `verification-loop`, `strategic-compact`, and `context-budget` for verification habits, long-task context management, and context auditing
- `ai-security` for checks on model artifacts, prompts, datasets, and logs (secrets, PII, unsafe deserialization, supply-chain risk)
- `cost-latency-optimization` for routing LLM/embedding workloads by quality, latency, and budget
- `llm-observability` for logging, tracing, and monitoring of ML/LLM serving paths
- `agent-evaluation` and `agent-self-evaluation` for structured scoring of model, pipeline, or agent output

## Reuse the Shipped Review Surface

Do not treat MLE as separate from software engineering. The review agents and workflow
skills in this package apply directly to ML systems, often with stricter failure modes.

| Review surface | MLE use |
|----------------|---------|
| `ai-architect` / `rag-pipeline-reviewer` | Scope model changes as product capabilities; review retrieval, embedding, chunking, reranking, and retrieval-evaluation concerns |
| `mle-workflow` / `mle-reviewer` | Turn model work into production contracts: data, eval, serving, monitoring, rollback |
| `eval-harness` / `ai-regression-testing` | Turn offline metrics, slice checks, latency budgets, and rollback drills into repeatable gates; preserve every bug as a regression |
| `verification-loop` / `strategic-compact` / `context-budget` | Verify MLE changes before completion; split long ML work into parallel tracks; keep durable context |
| `ai-security` / `ai-security-reviewer` | Check model artifacts, notebooks, prompts, datasets, and logs for secrets, PII, unsafe deserialization, and supply-chain risk |
| `agent-evaluator` / `agent-evaluation` | Score model, pipeline, or agent output against a structured rubric |
| `llm-observability` / `cost-latency-optimization` | Monitor serving health and quality; route LLM/embedding workloads by quality, latency, and budget |
| `code-architect` | Produce feature-plan blueprints with concrete files, interfaces, data flow, and build order |
...
| ID | Common MLE task | Streamlined path | Required output | Pipeline lanes covered |
|----|-----------------|------------------|-----------------|------------------------|
| MLE-01 | Frame an ambiguous prediction, ranking, recommender, classifier, embedding, or forecast capability | `ai-architect`, `mle-workflow` | Iteration Compact naming who cares, decision owner, success metric, unacceptable mistakes, assumptions, constraints, and first experiment | product contract, stakeholder loss, risk, rollout |
| MLE-02 | Define metric goals, labels, data sources, and the mistake budget | `mle-workflow`, `mle-reviewer` | Data and metric contract with entity grain, label timing, label confidence, feature timing, point-in-time joins, split policy, and dataset snapshot | data contract, metric design, leakage, reproducibility |
| MLE-03 | Build a baseline model and scoring path before adding complexity | `mle-reviewer`, `eval-harness` | Baseline scorer with confusion matrix, calibration notes, latency/cost estimate, known weaknesses, and tests for score shape and determinism | baseline, scoring, testing, serving parity |
| MLE-04 | Generate features from hypotheses about what separates outcomes | `mle-workflow`, `code-architect` | Feature plan and transform module covering signal source, missing values, outliers, correlations, leakage checks, and train/serve equivalence | feature pipeline, leakage, training, artifacts |
| MLE-05 | Tune thresholds, configs, and model complexity under tradeoffs | `eval-harness`, `ai-regression-testing` | Threshold/config report comparing precision, recall, F1, AUC, calibration, group slices, latency, cost, complexity, and acceptable error classes | evaluation, threshold, promotion, regression |
| MLE-06 | Run error analysis and turn mistakes into the next experiment | `eval-harness`, `mle-reviewer` | Error cluster report for false positives, false negatives, ambiguous labels, stale features, missing signals, and bug traces with lessons captured | error analysis, bug trace, iteration, regression |
| MLE-07 | Package a model artifact for batch or online inference | `ai-security`, `mle-reviewer` | Versioned artifact bundle with preprocessing, config, dependency constraints, schema validation, safe loading, and PII-safe logs | artifact, security, inference contract |
| MLE-08 | Ship online serving or batch scoring with feedback capture | `ai-architect`, `llm-observability` | Prediction endpoint or batch job with response envelope, timeout, batching, fallback, model version, confidence, feedback logging, and product-flow tests | serving, batch inference, fallback, user workflow |
| MLE-09 | Roll out a model with shadow traffic, canary, A/B test, or rollback | `mle-reviewer`, `verification-loop` | Rollout plan naming traffic split, dashboards, p95 latency, cost, quality guardrails, rollback artifact, and rollback trigger | deployment, canary, rollback |
| MLE-10 | Operate, debug, and refresh a production model after launch | `mle-workflow`, `llm-observability`, `mle-reviewer` | Observation ledger and refresh plan with drift checks, delayed-label health, alert owners, runbook updates, retrain criteria, and PR evidence | monitoring, incident response, retraining |
