...
- Running a bug-check review pass after code changes
...
## Integrating Tests into the Bug-Check Workflow

### Manual Workflow

This package does not ship a bug-check command; run the workflow steps directly. The
key rule: automated tests run FIRST, before any review judgment:
...
To make this repeatable, you can define your own OpenCode command (a `command` entry in
`opencode.json` with a `{file:...}` template) that embeds the steps above.

### The Workflow

```
User: "run a bug check"
  │
  ├─ Step 1: npm run test
